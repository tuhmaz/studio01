...
-- Adds audit logging and supporting production indexes.

CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- Broaden role constraint for the permission layer.
ALTER TABLE public.users DROP CONSTRAINT IF EXISTS users_role_check;
ALTER TABLE public.users
  ADD CONSTRAINT users_role_check
  CHECK (role IN ('SUPER_ADMIN', 'ADMIN', 'MANAGER', 'ACCOUNTANT', 'LEADER', 'WORKER'));

-- Optional profile/payroll columns used by newer app builds.
